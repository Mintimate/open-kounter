...
Open Kounter 是一个灵活的计数器服务，旨在替代 LeanCloud 为静态网站（如 Hexo）提供 PV/UV 统计功能。它包含一个完整的管理后台，支持数据管理、导入导出、域名白名单和 Passkey 无密码登录。

**支持两种部署方式：**
- **EdgeOne Pages 部署**：基于 EdgeOne Pages Functions 和 KV 存储的无服务器部署
- **本地服务器部署**：基于 Node.js + Express + SQLite 的私有化部署
...
如果你希望在自己的服务器上部署 Open Kounter，可以使用本地部署方式。
...
1. **克隆项目**
   ```bash
   git clone https://github.com/Mintimate/open-kounter.git
   cd open-kounter
   ```
...
3. **配置环境变量**（可选）
   ...
   # 数据库路径（默认 ./data/openkounter.db）
   DB_PATH=./data/openkounter.db
   
   # 管理员 Token（可选，首次访问时设置）
...
├── edge-functions/         # EdgeOne Functions 后端逻辑
│   └── api/
│       ├── auth.js         # 认证逻辑
│       ├── counter.js      # 核心计数器逻辑
│       ├── init.js         # 初始化接口
│       └── passkey.js      # Passkey 相关逻辑
├── server/                 # 本地服务器部署（Node.js + Express）
│   ├── handlers/           # API 处理器
│   │   ├── auth.js         # 认证逻辑
│   │   ├── counter.js      # 核心计数器逻辑
│   │   ├── init.js         # 初始化接口
